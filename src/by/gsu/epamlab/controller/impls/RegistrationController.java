...
public class RegistrationController extends AbstractController {
    ...
    protected void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(login.isEmpty() && password.isEmpty() && confirmPassword.isEmpty()){
            jumpError(REGISTRATION_PAGE, EMPTY_LOGIN_AND_PASSWORD, request, response);
            return;
        }
        if(login.isEmpty()){
            jumpError(REGISTRATION_PAGE, EMPTY_LOGIN, request, response);
            return;
        }
        if(password.isEmpty()){
            jumpError(REGISTRATION_PAGE, EMPTY_PASSWORD, request, response);
            return;
        }
        if(confirmPassword.isEmpty()){
            jumpError(REGISTRATION_PAGE, EMPTY_CONFIRM_PASSWORD, request, response);
            return;
        }
        if (!password.equals(confirmPassword)){
            jumpError(REGISTRATION_PAGE, CONFIRM_NOT_EQUAL_TO_PASSWORD, request, response);
            return;
        }
